...
# The groups that should be installed
INSTGROUPS := APPS LIBS DATA DOCS $(INSTGROUPS)
APPS.dir = apps
...
$(eval $(foreach x,$(foreach y,$(GROUPS),$($y)),$(call RULES.MODULE,$x)))

# Generate the installation rules for given module
# $1 - module name
define INSTRULES.MODULE
$(if $(findstr $1,$(NO-INSTALL)),,\
.PHONY: install-$1
install: install-$1
$(foreach 2,$(TARGETS.$1),
$(call MKIRULES.$(.TKNAME),$1,$2,$(call XFNAME.$(.TKNAME),$2))
))
endef

# Evaluate install rules for installable modules
$(eval $(foreach x,$(foreach y,$(INSTGROUPS),$($y)),$(call INSTRULES.MODULE,$x)))

# Debug target to look at autogenerated rules
showrules:
	@echo -e '$(subst $(NL),\n,$(foreach x,$(foreach y,$(GROUPS),$($y)),$(call RULES.MODULE,$x)))'
	@echo -e '$(subst $(NL),\n,$(foreach x,$(foreach y,$(INSTGROUPS),$($y)),$(call INSTRULES.MODULE,$x)))'
