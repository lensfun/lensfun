...
#if defined (_MSC_VER)
#include <intrin.h>
guint _legacy_lf_detect_cpu_features ()
{
    ...
    if (cpuflags == (guint)-1)
    {
        ...
        if (CPUInfo [0] >= 1)
        {
            ...
            if (CPUInfo [3] & 0x800000)
                cpuflags |= LEGACY_LF_CPU_FLAG_MMX;
            if (CPUInfo [3] & 0x2000000)
                cpuflags |= LEGACY_LF_CPU_FLAG_SSE;
            if (CPUInfo [3] & 0x4000000)
                cpuflags |= LEGACY_LF_CPU_FLAG_SSE2;
            if (CPUInfo [3] & 0x8000)
                cpuflags |= LEGACY_LF_CPU_FLAG_CMOV;
            if (CPUInfo [2] & 0x1)
                cpuflags |= LEGACY_LF_CPU_FLAG_SSE3;
            if (CPUInfo [2] & 0x200)
                cpuflags |= LEGACY_LF_CPU_FLAG_SSSE3;
            if (CPUInfo [2] & 0x80000)
                cpuflags |= LEGACY_LF_CPU_FLAG_SSE4_1;
            if (CPUInfo [2] & 0x100000)
                cpuflags |= LEGACY_LF_CPU_FLAG_SSE4_2;
            ...
            if (CPUInfo [0] >= 1)
            {
                ...
                if (CPUInfo [3] & 0x80000000)
                    cpuflags |= LEGACY_LF_CPU_FLAG_3DNOW;
                if (CPUInfo [3] & 0x40000000)
                    cpuflags |= LEGACY_LF_CPU_FLAG_3DNOW_EXT;
                if (CPUInfo [3] & 0x00400000)
                    cpuflags |= LEGACY_LF_CPU_FLAG_AMD_ISSE;
            };
        };
    };
#if defined(GLIB_CHECK_VERSION) && GLIB_CHECK_VERSION(2,32,0)
