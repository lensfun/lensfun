...
void legacy_lfModifier::AddCoordCallback (
    legacy_lfModifyCoordFunc callback, int priority, void *data, size_t data_size)
{
    legacy_lfCoordCallbackData *d = new legacy_lfCoordCallbackData ();
    d->callback = callback;
    AddCallback (CoordCallbacks, d, priority, data, data_size);
}

bool legacy_lfModifier::AddCoordCallbackDistortion (legacy_lfLensCalibDistortion &model, bool reverse)
{
    float tmp [3];
    ...
    if (reverse)
        switch (model.Model)
        {
            ...
            case LEGACY_LF_DIST_MODEL_PTLENS:
                ...
                if (_legacy_lf_detect_cpu_features () & LEGACY_LF_CPU_FLAG_SSE)
...
double legacy_lfModifier::AutoscaleResidualDistance (float *coord) const
{
...
float legacy_lfModifier::GetTransformedDistance (legacy_lfPoint point) const
{
    ...
    for (int countdown = 50; ; countdown--)
    {
        ...
        {
            legacy_lfCoordCallbackData *cd =
                (legacy_lfCoordCallbackData *)g_ptr_array_index ((GPtrArray *)CoordCallbacks, j);
...
float legacy_lfModifier::GetAutoScale (bool reverse)
{
...
bool legacy_lfModifier::AddCoordCallbackScale (float scale, bool reverse)
{
    float tmp [1];
...
bool legacy_lfModifier::AddCoordCallbackGeometry (legacy_lfLensType from, legacy_lfLensType to, float focal)
{
    float tmp [2];
    ...
    if(from == to)
        return false;
    if(from == LEGACY_LF_UNKNOWN)
        return false;
    if(to == LEGACY_LF_UNKNOWN)
        return false;
...
bool legacy_lfModifier::ApplyGeometryDistortion (
    float xu, float yu, int width, int height, float *res) const
{
    ...
    for (float y = yu; height; y += NormScale, height--)
    {
        ...
        {
            legacy_lfCoordCallbackData *cd =
                (legacy_lfCoordCallbackData *)g_ptr_array_index ((GPtrArray *)CoordCallbacks, i);
...
void legacy_lfModifier::ModifyCoord_Scale (void *data, float *iocoord, int count)
{
...
void legacy_lfModifier::ModifyCoord_UnDist_Poly3 (void *data, float *iocoord, int count)
{
...
void legacy_lfModifier::ModifyCoord_Dist_Poly3 (void *data, float *iocoord, int count)
{
    // Rd = Ru * (1 - k1 + k1 * Ru^2)
...
void legacy_lfModifier::ModifyCoord_UnDist_Poly5 (void *data, float *iocoord, int count)
{
...
void legacy_lfModifier::ModifyCoord_Dist_Poly5 (void *data, float *iocoord, int count)
{
    float *param = (float *)data;
...
void legacy_lfModifier::ModifyCoord_UnDist_PTLens (void *data, float *iocoord, int count)
{
    float *param = (float *)data;
    float a = param [0];
    float b = param [1];
...
void legacy_lfModifier::ModifyCoord_Dist_PTLens (void *data, float *iocoord, int count)
{
    float *param = (float *)data;
...
void legacy_lfModifier::ModifyCoord_Geom_FishEye_Rect (void *data, float *iocoord, int count)
{
...
void legacy_lfModifier::ModifyCoord_Geom_Rect_FishEye (void *data, float *iocoord, int count)
{
    const float inv_dist = ((float *)data) [1];
...
void legacy_lfModifier::ModifyCoord_Geom_Panoramic_Rect (
...
void legacy_lfModifier::ModifyCoord_Geom_Rect_Panoramic (
    void *data, float *iocoord, int count)
{
    const float dist = ((float *)data) [0];
    const float inv_dist = ((float *)data) [1];
...
void legacy_lfModifier::ModifyCoord_Geom_FishEye_Panoramic (
    void *data, float *iocoord, int count)
{
    const float dist = ((float *)data) [0];
    const float inv_dist = ((float *)data) [1];
...
void legacy_lfModifier::ModifyCoord_Geom_Panoramic_FishEye (
    void *data, float *iocoord, int count)
{
    const float dist = ((float *)data) [0];
    const float inv_dist = ((float *)data) [1];
...
void legacy_lfModifier::ModifyCoord_Geom_ERect_Rect (void *data, float *iocoord, int count)
{
    const float dist = ((float *)data) [0];
    const float inv_dist = ((float *)data) [1];
...
void legacy_lfModifier::ModifyCoord_Geom_Rect_ERect (void *data, float *iocoord, int count)
{
    const float dist = ((float *)data) [0];
...
void legacy_lfModifier::ModifyCoord_Geom_ERect_FishEye (void *data, float *iocoord, int count)
{
    const float dist = ((float *)data) [0];
    const float inv_dist = ((float *)data) [1];
...
void legacy_lfModifier::ModifyCoord_Geom_FishEye_ERect (void *data, float *iocoord, int count)
{
    const float dist = ((float *)data) [0];
    const float inv_dist = ((float *)data) [1];
...
void legacy_lfModifier::ModifyCoord_Geom_ERect_Panoramic (void *data, float *iocoord, int count)
{
    const float dist = ((float *)data) [0];
    const float inv_dist = ((float *)data) [1];
...
void legacy_lfModifier::ModifyCoord_Geom_Panoramic_ERect (void *data, float *iocoord, int count)
{
    const float dist = ((float *)data) [0];
    const float inv_dist = ((float *)data) [1];
...
void legacy_lfModifier::ModifyCoord_Geom_Orthographic_ERect (void *data, float *iocoord, int count)
{
    const float dist = ((float *)data) [0];
    const float inv_dist = ((float *)data) [1];
...
void legacy_lfModifier::ModifyCoord_Geom_ERect_Orthographic (void *data, float *iocoord, int count)
{
    const float dist = ((float *)data) [0];
    const float inv_dist = ((float *)data) [1];
...
void legacy_lfModifier::ModifyCoord_Geom_Stereographic_ERect (void *data, float *iocoord, int count)
{
    const float dist = ((float *)data) [0];
    const float inv_dist = ((float *)data) [1];
    for (float *end = iocoord + count * 2; iocoord < end; iocoord += 2)
...
void legacy_lfModifier::ModifyCoord_Geom_ERect_Stereographic (void *data, float *iocoord, int count)
{
    const float dist = ((float *)data) [0];
    const float inv_dist = ((float *)data) [1];
...
void legacy_lfModifier::ModifyCoord_Geom_Equisolid_ERect (void *data, float *iocoord, int count)
{
    const float dist = ((float *)data) [0];
    const float inv_dist = ((float *)data) [1];
    for (float *end = iocoord + count * 2; iocoord < end; iocoord += 2)
...
void legacy_lfModifier::ModifyCoord_Geom_ERect_Equisolid (void *data, float *iocoord, int count)
{
    const float dist = ((float *)data) [0];
    for (float *end = iocoord + count * 2; iocoord < end; iocoord += 2)
    {
...
void legacy_lfModifier::ModifyCoord_Geom_Thoby_ERect (void *data, float *iocoord, int count)
{
    const float dist = ((float *)data) [0];
    const float inv_dist = ((float *)data) [1];
    for (float *end = iocoord + count * 2; iocoord < end; iocoord += 2)
...
void legacy_lfModifier::ModifyCoord_Geom_ERect_Thoby (void *data, float *iocoord, int count)
{
    const float dist = ((float *)data) [0];
    const float inv_dist = ((float *)data) [1];
    for (float *end = iocoord + count * 2; iocoord < end; iocoord += 2)
...
void legacy_lf_modifier_add_coord_callback (
...
int legacy_lf_modifier_add_coord_callback_distortion (
    legacy_lfModifier *modifier, legacy_lfLensCalibDistortion *model, int reverse)
{
    return modifier->AddCoordCallbackDistortion (*model, reverse);
}

int legacy_lf_modifier_add_coord_callback_geometry (
...
int legacy_lf_modifier_add_coord_callback_scale (
    legacy_lfModifier *modifier, float scale, int reverse)
{
    return modifier->AddCoordCallbackScale (scale, reverse);
}

float legacy_lf_modifier_get_auto_scale (legacy_lfModifier *modifier, int reverse)
{
    return modifier->GetAutoScale (reverse);
}

int legacy_lf_modifier_apply_geometry_distortion (
