...
static const char *_legacy_lf_get_lang ()
{
#ifndef LC_MESSAGES
...
LEGACY_LF_EXPORT const char *legacy_lf_mlstr_get (const legacy_lfMLstr str)
{
    ...
    const char *lang = _legacy_lf_get_lang ();
...
void _legacy_lf_list_free (void **list)
{
    int i;
...
void _legacy_lf_setstr (gchar **var, const gchar *val)
{
    if (*var)
        g_free (*var);
...
void _legacy_lf_addstr (gchar ***var, const gchar *val)
{
    _legacy_lf_addobj ((void ***)var, val, strlen (val) + 1, NULL);
}

void _legacy_lf_addobj (void ***var, const void *val, size_t val_size,
...
bool _legacy_lf_delobj (void ***var, int idx)
{
    if (!(*var))
        return false;
...
void _legacy_lf_xml_printf (GString *output, const char *format, ...)
{
    va_list args;
...
void _legacy_lf_xml_printf_mlstr (GString *output, const char *prefix,
                           const char *element, const legacy_lfMLstr val)
{
    if (!val)
        return;

    _legacy_lf_xml_printf (output, "%s<%s>%s</%s>\n", prefix, element, val, element);
    ...
    for (const char *cur = val;;)
    {
        ...
        const char *lang = cur;
        cur = strchr (cur, 0) + 1;
        _legacy_lf_xml_printf (output, "%s<%s lang=\"%s\">%s</%s>\n",
                        prefix, element, lang, cur, element);
    }
}

int _legacy_lf_ptr_array_insert_sorted (
...
int _legacy_lf_ptr_array_insert_unique (
    GPtrArray *array, void *item, GCompareFunc compare, GDestroyNotify dest)
{
    int idx1, idx2;
    int idx = _legacy_lf_ptr_array_insert_sorted (array, item, compare);
    void **root = array->pdata;
    int length = array->len;
...
int _legacy_lf_ptr_array_find_sorted (
...
int _legacy_lf_strcmp (const char *s1, const char *s2)
{
    if (s1 && !*s1)
        s1 = NULL;
    if (s2 && !*s2)
...
int _legacy_lf_mlstrcmp (const char *s1, const legacy_lfMLstr s2)
{
    if (!s1)
    {
        if (!s2)
    ...
    while (*s2c)
    {
        int res = _legacy_lf_strcmp (s1, s2c);
        if (!res)
            return 0;
        if (s2c == s2)
            ret = res;
...
float _legacy_lf_interpolate (float y1, float y2, float y3, float y4, float t)
{
    float tg2, tg3;
    float t2 = t * t;
    float t3 = t2 * t;
...
long int _legacy_lf_read_database_timestamp (const gchar *dirname)
{
...
legacy_lfFuzzyStrCmp::legacy_lfFuzzyStrCmp (const char *pattern, bool allwords)
{
...
legacy_lfFuzzyStrCmp::~legacy_lfFuzzyStrCmp ()
{
    Free (pattern_words);
...
void legacy_lfFuzzyStrCmp::Free (GPtrArray *dest)
{
...
void legacy_lfFuzzyStrCmp::Split (const char *str, GPtrArray *dest)
{
    if (!str)
        return;
    ...
    while (*str)
    {
        ...
        _legacy_lf_ptr_array_insert_sorted (dest, item, (GCompareFunc)strcmp);
    }
}

int legacy_lfFuzzyStrCmp::Compare (const char *match)
{
...
int legacy_lfFuzzyStrCmp::Compare (const legacy_lfMLstr match)
{
    if (!match)
        return 0;
