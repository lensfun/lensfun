...
#ifndef __LEGACY_LENSFUNPRV_H__
#define __LEGACY_LENSFUNPRV_H__
...
// halegacy_lf of the smallest image dimension (width or height)
// adjusted for the lens calibration data/camera crop factors.
#define NEWTON_EPS 0.00001

class legacy_lfFuzzyStrCmp;
 ...
 */
extern void _legacy_lf_list_free (void **list);
 ...
 */
extern void _legacy_lf_setstr (gchar **var, const gchar *val);
 ...
 */
extern void _legacy_lf_addstr (gchar ***var, const gchar *val);
 ...
 */
extern int _legacy_lf_ptr_array_insert_sorted (
 ...
 *     The index at which the item was inserted.
 */
extern int _legacy_lf_ptr_array_insert_unique (
 ...
 */
extern int _legacy_lf_ptr_array_find_sorted (
 ...
 */
extern bool _legacy_lf_delobj (void ***var, int idx);
 ...
 *     The format string.
 */
extern void _legacy_lf_xml_printf (GString *output, const char *format, ...);
 ...
 */
extern void _legacy_lf_xml_printf_mlstr (GString *output, const char *prefix,
                                  const char *element, const legacy_lfMLstr val);
 ...
 *     The model.
 */
extern const char *_legacy_lf_get_distortion_model_id (legacy_lfDistortionModel model);
 ...
 */
extern int _legacy_lf_strcmp (const char *s1, const char *s2);

/**
 * @brief Same as _legacy_lf_strcmp(), but compares a string with a multi-language
 * string.
 *
 ...
 */
extern int _legacy_lf_mlstrcmp (const char *s1, const legacy_lfMLstr s2);
 ...
 * @param a
 *     A pointer to first legacy_lfMount object.
 * @param b
 *     A pointer to second legacy_lfMount object.
 * @return
 *     Positive if a > b, negative if a < b, zero if they are equal.
 */
extern gint _legacy_lf_mount_compare (gconstpointer a, gconstpointer b);
 ...
 * @param a
 *     A pointer to first legacy_lfCamera object.
 * @param b
 *     A pointer to second legacy_lfCamera object.
 * @return
 *     Positive if a > b, negative if a < b, zero if they are equal.
 */
extern gint _legacy_lf_camera_compare (gconstpointer a, gconstpointer b);
 ...
 * functions like _legacy_lf_lens_compare.
 * @param i1
 *     A pointer to first legacy_lfLens object.
 * @param i2
 *     A pointer to second legacy_lfLens object.
 * @return
 *     Positive if i1 > i2, negative if i1 < i2, zero if they are equal.
 */
extern gint _legacy_lf_lens_parameters_compare (const legacy_lfLens *i1, const legacy_lfLens *i2);
 ...
 * be used by such sorting key functions like _legacy_lf_lens_compare.
 * @param i1
 *     A pointer to first legacy_lfLens object.
 * @param i2
 *     A pointer to second legacy_lfLens object.
 ...
 *     Positive if i1 > i2, negative if i1 < i2, zero if they are equal.
 */
extern gint _legacy_lf_lens_name_compare (const legacy_lfLens *i1, const legacy_lfLens *i2);
 ...
 *     Positive if a > b, negative if a < b, zero if they are equal.
 */
extern gint _legacy_lf_lens_compare (gconstpointer a, gconstpointer b);
 ...
 */
extern float _legacy_lf_interpolate (float y1, float y2, float y3, float y4, float t);
 ...
 *     updated.
 */
extern long int _legacy_lf_read_database_timestamp(const gchar *dirname);
 ...
 */
extern int _legacy_lf_lens_compare_score (const legacy_lfLens *pattern, const legacy_lfLens *match,
                                   legacy_lfFuzzyStrCmp *fuzzycmp, const char **compat_mounts);

enum
{
    LEGACY_LF_CPU_FLAG_MMX             = 0x00000001,
    LEGACY_LF_CPU_FLAG_SSE             = 0x00000002,
    LEGACY_LF_CPU_FLAG_CMOV            = 0x00000004,
    LEGACY_LF_CPU_FLAG_3DNOW           = 0x00000008,
    LEGACY_LF_CPU_FLAG_3DNOW_EXT       = 0x00000010,
    LEGACY_LF_CPU_FLAG_AMD_ISSE        = 0x00000020,
    LEGACY_LF_CPU_FLAG_SSE2            = 0x00000040,
    LEGACY_LF_CPU_FLAG_SSE3            = 0x00000080,
    LEGACY_LF_CPU_FLAG_SSSE3           = 0x00000100,
    LEGACY_LF_CPU_FLAG_SSE4_1          = 0x00000200,
    LEGACY_LF_CPU_FLAG_SSE4_2          = 0x00000400
};
 ...
 */
extern guint _legacy_lf_detect_cpu_features ();
...
public:
     ...
     */
    legacy_lfFuzzyStrCmp (const char *pattern, bool allwords);
    ~legacy_lfFuzzyStrCmp ();
     ...
     *     in match.
     */
    int Compare (const legacy_lfMLstr match);
};

/// Subpixel distortion callback
struct legacy_lfSubpixelCallbackData : public legacy_lfCallbackData
{
    legacy_lfSubpixelCoordFunc callback;
};

/// A single pixel coordinate modifier callback.
struct legacy_lfCoordCallbackData : public legacy_lfCallbackData
{
    legacy_lfModifyCoordFunc callback;
};

/// A single pixel color modifier callback.
struct legacy_lfColorCallbackData : public legacy_lfCallbackData
{
    legacy_lfModifyColorFunc callback;
};
