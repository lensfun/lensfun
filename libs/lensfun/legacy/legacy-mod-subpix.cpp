...
void legacy_lfModifier::AddSubpixelCallback (
    legacy_lfSubpixelCoordFunc callback, int priority, void *data, size_t data_size)
{
    legacy_lfSubpixelCallbackData *d = new legacy_lfSubpixelCallbackData ();
    d->callback = callback;
    AddCallback (SubpixelCallbacks, d, priority, data, data_size);
}

bool legacy_lfModifier::AddSubpixelCallbackTCA (legacy_lfLensCalibTCA &model, bool reverse)
{
    float tmp [2];
...
bool legacy_lfModifier::ApplySubpixelDistortion (
    float xu, float yu, int width, int height, float *res) const
{
    ...
    for (float y = yu; height; y += NormScale, height--)
    {
        ...
        {
            legacy_lfSubpixelCallbackData *cd =
                (legacy_lfSubpixelCallbackData *)g_ptr_array_index ((GPtrArray *)SubpixelCallbacks, i);
...
bool legacy_lfModifier::ApplySubpixelGeometryDistortion (
    float xu, float yu, int width, int height, float *res) const
{
...
void legacy_lfModifier::ModifyCoord_UnTCA_Linear (void *data, float *iocoord, int count)
{
...
void legacy_lfModifier::ModifyCoord_TCA_Linear (void *data, float *iocoord, int count)
{
    float *param = (float *)data;
    float k_r = param [0];
    float k_b = param [1];
...
void legacy_lfModifier::ModifyCoord_UnTCA_Poly3 (void *data, float *iocoord, int count)
{
...
void legacy_lfModifier::ModifyCoord_TCA_Poly3 (void *data, float *iocoord, int count)
{
    // Rd = Ru * (b * Ru^2 + c * Ru + v)
    const float *param = (float *)data;
    const float vr = param [0];
...
void legacy_lf_modifier_add_subpixel_callback (
...
cbool legacy_lf_modifier_add_subpixel_callback_TCA (
    legacy_lfModifier *modifier, legacy_lfLensCalibTCA *model, cbool reverse)
{
    return modifier->AddSubpixelCallbackTCA (*model, reverse);
}

cbool legacy_lf_modifier_apply_subpixel_distortion (
...
cbool legacy_lf_modifier_apply_subpixel_geometry_distortion (
    legacy_lfModifier *modifier, float xu, float yu, int width, int height, float *res)
{
